...
requiring a unintuitive and performance-degrading interaction with the existing [module scope operator](https://dlang.org/spec/module.html#module_scope_operators)
(`.x` would have to prioritise module-level variables over enum members) and possibly limiting the feature's future potential.

`..member` was deemed to blend in too well with the range operator and other existing uses of dots in D. 
